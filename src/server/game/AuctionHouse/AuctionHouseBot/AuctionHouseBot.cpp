...
uint32 AuctionBotSeller::SetStat(AHB_Seller_Config& config)
{
    ...
    for (uint32 j=0; j<MAX_AUCTION_QUALITY; ++j)
    {
        for (uint32 i=0;i<MAX_ITEM_CLASS;++i)
        {
            // sLog->outString("ItemsInAH [%u]", ItemsInAH[j][i]);
    ...
    for (uint32 i=0; i<MAX_ITEM_CLASS;++i)
    {
            // sLog->outString("AHBot: %-18s\t%u\t%u\t%u\t%u\t%u\t%u\t%u",
            sLog->outString("AHBot: %-14s\t%u\t%u\t%u\t%u\t%u\t%u\t%u",
            sAuctionBotConfig.GetItemClassName(ItemClass(i)),
    ...
    // sLog->outString("Show Count [%u]", count);
    return count;
}
...
void AuctionBotSeller::addNewAuctions(AHB_Seller_Config& config)
{
    ...
    //else if(config.LastMissedItem > sAuctionBotConfig.GetItemPerCycleNormal())
/*    else
    {
         items = config.LastMissedItem;
         sLog->outString("AHBot: Lower value used to fill AH");
    } */
    uint32 houseid;
